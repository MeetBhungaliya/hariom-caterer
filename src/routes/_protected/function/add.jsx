...
function RouteComponent() {
  ...
  const bomBoysTotal = useStore(store, (state) => state.values.bom_boys.total);
  const decorationTotal = useStore(
    store,
    (state) => state.values.decoration.total
  );

  const toNumber = (value) => (isNaN(value) ? 0 : Number(value));

  const getTotalCost = useCallback(() => {
    const totalOfData = data.reduce(
  ...
  const addFunctionMutation = useMutation({
    mutationFn: async (data) =>
      fetchApi({ url: ADD_FUNCTION, method: METHODS.POST, data }),
  });

  async function onSubmit({ value }) {
    const filterdData = value.data.filter(
      (item) => item.date && item.function && item.person && item.rate
    );
    const payload = {
      ...value,
      data: filterdData,
    ...
    const result = await asyncResponseToaster(() =>
      addFunctionMutation.mutateAsync(payload)
    );
